...
public class EmpRepository implements IEmpRepository {
	...
	public EmpVO getEmpInfo(int empId) {
		String sql = "select employee_id, first_name, last_name, email, phone_number,"
				...
				+ "manager_name, e.department_id, department_name, emp_pic, file_size " + "from employees e " + "left join jobs j "
				+ "on e.job_id=j.job_id " + "left join departments d " + "on e.department_id=d.department_id "
				...
				+ "on e.manager_id=m.manager_id " + "where employee_id=?";
	...
	public List<EmpVO> selectName(String nameId) {
		String sql = "select * from employees where first_name like ? or last _name like ?";
		return jdbcTemplate.query(sql, empMapper, nameId, nameId);
	}

	@Override
	public void updateManagers(int empId) {
		String sql = "update (selet * from employees where manager_id=?) set manager_id=null";
		jdbcTemplate.update(sql, empId);
	...
	public int idCheck(int empId) {
		String sql = "select count(employee_id) from employees where employee_id=?";
		return 0;
	}
